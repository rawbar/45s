"""
endgame_eval.py — test the OPT-IN 'endgame_deny' bidder-side rule in a
GAME context (full games to 120, real team scores).

Champion baseline = Policy() (rule absent → bit-identical faithful port).
Challenger        = REGISTRY['endgame_deny'] (rule on).

Rule only fires when the challenger team is the BIDDER and the opponent is
at game point (oppNeed = 120 - oppScore <= 10). When it never fires the
two policies are identical, so any overall win-rate delta is attributable
purely to endgame play. We also stratify on the challenger team's CLOSEST
opponent need across the game:

   trig   closest opp need <= 10   (the rule's trigger regime)
   near   11..20                   (almost-trigger; should be ~neutral)
   safe   > 20                     (rule never fires → must be ~50%)

Paired mirrored deals (same seed, swap which team is challenger) so deal
luck cancels. Winner = positive overall AND positive in `trig` (z>=1.96)
AND not significantly regressing overall; re-validated on disjoint seeds.

Run:  python -m bidding_simulator_v2.endgame_eval --deals 8000 --holdout 40000
"""
...
CHAMP = Policy()
CHAL = REGISTRY['endgame_deny']


def _bucket(need):
    if need <= 10:  return 0          # trig
    if need <= 20:  return 1          # near
    return 2                          # safe
...
def main():
    ...
    print('\n' + '=' * 78)
    print('  ENDGAME-DENY (opt-in) — game-context paired test vs champion')
    print('  trig=oppNeed<=10 (rule fires)  near=11-20  safe=>20 (no-op)')
    print('=' * 78)
    t0 = time.time()
    ...
    print('=' * 78)
    if not ok_screen:
        print("  NO WIN at screen: endgame-deny does not significantly beat "
    ...
    print('=' * 78)
    print("  VERDICT: " + ("CONFIRMED — endgame-deny helps the trigger "
